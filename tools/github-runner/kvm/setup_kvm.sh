...
for node in "${!RUNNERS[@]}"; do
    ssh root@${RUNNERS[$node]} "sudo apt update && sudo apt -y install wget curl unzip qemu-kvm libvirt-daemon  bridge-utils virtinst libvirt-daemon-system virt-top libguestfs-tools libosinfo-bin  qemu-system virt-manager"
