...
for node in "${RUNNERS[@]}"; do
    ...
    until [ $RUNNERS_PER_NODE -eq 0 ]; do
        ssh root@$node "docker run -d --network host --restart always --name github-runner$RUNNER_COUNT -e REPO_URL="https://github.com/cncf/cnf-testsuite" -e RUNNER_NAME="runner$RUNNER_COUNT" -e RUNNER_TOKEN="$TOKEN" -e RUNNER_WORKDIR="/tmp/github-runner-cnf-testsuite" -e RUNNER_GROUP="testsuite" -v /var/run/docker.sock:/var/run/docker.sock -v /tmp/github-runner-your-repo:/tmp/github-runner-your-repo -v \$(pwd)/shared:/shared conformance/github-runner:latest"
