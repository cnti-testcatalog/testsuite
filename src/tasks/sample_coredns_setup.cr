...
task "sample_privileged_cnf_setup", ["helm_local_install"] do |_, args|
  current_dir = FileUtils.pwd 
  puts current_dir if check_verbose(args)
  ...
  # TODO copy sample cnf directory into the ./cnfs directory
  yml_cp = `cp -r #{current_dir}/sample-cnfs/sample_privileged_cnf_setup_coredns #{current_dir}/#{CNF_DIR}/`
  puts yml_cp if check_verbose(args)
  ...
  begin
    ...
    helm = "#{current_dir}/#{TOOLS_DIR}/helm/linux-amd64/helm"
    puts helm if check_verbose(args)
    # TODO replace helm_chart with helm_directory
...
# TODO sample_privileged_cnf_cleanup
