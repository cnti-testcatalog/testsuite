...
task "install_falco" do |_, args|
  ...
  if args.named["offline"]?
    ...
    Helm.install("falco --set ebpf.enabled=true #{image_arg} #{image_tag} -f ./falco_rule.yaml #{FALCO_OFFLINE_DIR}/#{helm_chart}")
  else
    Helm.helm_repo_add("falcosecurity","https://falcosecurity.github.io/charts")
    # needs ebpf parameter for precompiled module 
    Helm.install("falco --set ebpf.enabled=true #{image_arg} #{image_tag} -f ./falco_rule.yaml falcosecurity/falco")
  end
end
