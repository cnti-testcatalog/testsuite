...
<<<<<<< HEAD:src/tasks/configuration_lifecycle.cr
...
#
#     # TODO all of these configs need to be looped on from the container_names field
...
#     helm_chart_container_name = config.get("helm_chart_container_name").as_s
#
#     # TODO this needs to be retrieved from k8s
#     helm_chart_values = get_helm_chart_values(args, release_name)
#     image_name = helm_chart_values["image"]["repository"]
...
task "rollback" do |_, args|
  task_runner(args) do |args|
    ...
    if update_applied && rollout_status
      upsert_passed_task("rollback","✔️  PASSED: CNF #{deployment_name} Rolling Update Passed" )
    else
