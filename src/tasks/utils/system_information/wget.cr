...
def wget_global_response(verbose=false)
  ...
  LOGGING.info wget_response if verbose
  wget_response
end
...
def wget_local_response(verbose=false)
  ...
  # wget_response = `#{wget} --version`
  Process.run("#{wget} --version", shell: true, output: wget_response = IO::Memory.new)
