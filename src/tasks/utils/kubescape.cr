...
module Kubescape
  ...
  def self.scan(cli : String | Nil = nil)
    if cli == nil
      ...
      cli = "framework nsa --use-from #{tools_dir}/kubescape/nsa.json --exclude-namespaces #{exclude_namespaces} --format json --output kubescape_results.json"
    end
    cmd = "#{tools_dir}/kubescape/kubescape scan #{cli}"
