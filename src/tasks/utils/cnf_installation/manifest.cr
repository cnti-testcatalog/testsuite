    ...
    def self.add_namespace_to_resources(manifest_string, namespace) 
      ...
      parsed_manifest.each do |resource|
        if resource["kind"].as_s.in?(list_of_namespaced_resources)
          ...
          Log.for("add_namespace_to_resource").info { "Added #{namespace} namespace for resource: {kind: #{resource["kind"]}, name: #{resource["metadata"]["name"]}}" }
        end
        ymls << resource
      end

      string_manifest_with_namespaces = combine_ymls_as_manifest_string(ymls)
      Log.for("add_namespace_to_resource").debug { "\n#{string_manifest_with_namespaces}" }
      string_manifest_with_namespaces
    end
