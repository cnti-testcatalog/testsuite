...
module CNFInstall
  ...
  def self.install_cnf(cli_args)
    ...
    prepare_deployment_directories(config)
  ...
  def self.parse_cli_args(cli_args)
    ...
    if args.named.keys.includes? "cnf-config"
      cnf_config_path = args.named["cnf-config"].as(String)
    elsif args.named.keys.includes? "cnf-path"
      cnf_config_path = args.named["cnf-path"].as(String)
    end
    cnf_config_path = self.ensure_cnf_config_path_file(cnf_config_path)

    if args.named.keys.includes? "timeout"
      timeout = args.named["timeout"].to_i
    end
  ...
  def self.prepare_deployment_directories(config, cnf_config_path)
    ...
    config.deployments.helm_chart.each do |helm_chart_config|
  ...
  def self.install_deployments(parsed_args, deployment_managers)
    deployment_managers.each do |deployment_manager|
      ...
      if !parsed_args[:skip_wait_for_install]
        wait_for_deployment_resources(deployment_name, generated_common_manifest, parsed_args[:timeout])
      end
    end
    stdout_success "CNF setup sucessfull."
  end

  def wait_for_deployment_resources(deployment_name, deployment_manifest, timeout)
    workload_resource_ymls = manifest_string_to_ymls(deployment_manifest).select { |resource| 
    ["replicaset", "deployment", "statefulset", "pod", "daemonset"].includes?(resource[:kind].downcase) 
    }
    total_resource_count = workload_resource_ymls.size()
    current_resource_number = 1
    workload_resource_ymls.each do | resource |
      ...
      ready = KubectlClient::Get.resource_wait_for_install(resource[:kind], resource[:name], wait_count: timeout, namespace: resource[:namespace])
      if !ready
        stdout_failure "\"#{deployment_name}\" deployment setup has timed-out, [#{resource[:kind]}] #{resource[:name]} is not ready after #{timeout} seconds.", same_line: true
