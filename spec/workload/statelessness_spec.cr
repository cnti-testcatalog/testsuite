...
describe "Statelessness" do
  ...
  it "'volume_hostpath_not_found' should fail if the cnf has a hostPath volume", tags: ["volume_hostpath_not_found"]  do
    begin
      `./cnf-testsuite cnf_setup cnf-config=sample-cnfs/sample-fragile-state/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      response_s = `./cnf-testsuite volume_hostpath_not_found verbose`
      LOGGING.info "Status:  #{response_s}"
      (/FAILED: hostPath volumes found/ =~ response_s).should_not be_nil
    ensure
      `./cnf-testsuite cnf_cleanup cnf-config=sample-cnfs/sample-fragile-state/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
    end
  end
  it "'no_local_volume_configuration' should fail if local storage configuration found", tags: ["no_local_volume_configuration"]  do
    begin
      ...
      `./cnf-testsuite cnf_setup cnf-config=sample-cnfs/sample-local-storage/cnf-conformance.yml verbose`
      $?.success?.should be_true
      ...
      `./cnf-testsuite cnf_cleanup cnf-config=sample-cnfs/sample-local-storage/cnf-conformance.yml deploy_with_chart=false`
  ...
  it "'no_local_volume_configuration' should pass if local storage configuration is not found", tags: ["no_local_volume_configuration"]  do
    begin
      `./cnf-testsuite cnf_setup cnf-config=sample-cnfs/sample-coredns-cnf/cnf-conformance.yml wait_count=0 verbose`
      $?.success?.should be_true
      response_s = `./cnf-testsuite no_local_volume_configuration verbose`
      LOGGING.info "Status:  #{response_s}"
      (/PASSED: local storage configuration volumes not found/ =~ response_s).should_not be_nil
    ensure
      `./cnf-testsuite cnf_cleanup cnf-config=sample-cnfs/sample-coredns-cnf/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
    end
  end
end
