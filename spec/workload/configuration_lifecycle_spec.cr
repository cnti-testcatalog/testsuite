...
describe CnfConformance do
  ...
  it "'liveness' should pass when livenessProbe is set", tags: ["liveness"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/k8s-multiple-deployments/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      ...
      LOGGING.info `./cnf-testsuite cnf_cleanup cnf-config=./sample-cnfs/k8s-multiple-deployments/cnf-conformance.yml deploy_with_chart=false `
    end
  end

  it "'liveness' should fail when livenessProbe is not set", tags: ["liveness"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_coredns_bad_liveness/cnf-conformance.yml verbose wait_count=0`
      $?.success?.should be_true
  ...
  it "'rolling_update' should pass when valid version is given", tags: ["rolling_update"]  do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_coredns/cnf-conformance.yml verbose wait_count=0`
      $?.success?.should be_true
  ...
  it "'rolling_update' should fail when invalid version is given", tags: ["rolling_update"]  do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_coredns_invalid_version/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      response_s = `./cnf-testsuite rolling_update verbose`
      LOGGING.info response_s
      $?.success?.should be_true
      ...
      LOGGING.info `./cnf-testsuite cnf_cleanup cnf-config=./sample-cnfs/sample_coredns_invalid_version/cnf-conformance.yml deploy_with_chart=false`
    end
  end
  ...
  it "'secrets_used' should pass when secrets are provided as volumes and used by a container", tags: ["secrets_used"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_secret_volume/cnf-conformance.yml verbose `
      $?.success?.should be_true
  ...
  it "'secrets_used' should fail when secrets are provided as volumes and not mounted by a container", tags: ["secrets_used"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_unmounted_secret_volume/cnf-conformance.yml verbose wait_count=0 `
      $?.success?.should be_true
      response_s = `./cnf-testsuite secrets_used verbose`
      LOGGING.info response_s
      $?.success?.should be_true
  ...
  it "'secrets_used' should pass when secrets are provided as environment variables and used by a container", tags: ["secrets_used"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_secret_env/cnf-conformance.yml verbose `
      $?.success?.should be_true
      response_s = `./cnf-testsuite secrets_used verbose`
      LOGGING.info response_s
      $?.success?.should be_true
  ...
  it "'secrets_used' should skip when secrets are not referenced as environment variables by a container", tags: ["secrets_used"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_secret_env_no_ref/cnf-conformance.yml verbose `
      $?.success?.should be_true
      response_s = `./cnf-testsuite secrets_used verbose`
      LOGGING.info response_s
      $?.success?.should be_true
  ...
  it "'immutable_configmap' fail without immutable configmaps", tags: ["immutable_configmap"] do
    begin
      `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_coredns/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      ...
      `./cnf-testsuite cnf_cleanup cnf-config=./sample-cnfs/sample_coredns/cnf-conformance.yml deploy_with_chart=false`
    end
  end
  ...
  it "'immutable_configmap' fail with only some immutable configmaps", tags: ["immutable_configmap"] do
    begin
      ...
      `./cnf-testsuite cnf_cleanup cnf-config=./sample-cnfs/sample_immutable_configmap_some/cnf-conformance.yml deploy_with_chart=false`
    end
  end

  it "'immutable_configmap' should pass with all immutable configmaps", tags: ["immutable_configmap"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_immutable_configmap_all/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      response_s = `./cnf-testsuite immutable_configmap verbose`
      LOGGING.info response_s
      $?.success?.should be_true
      ...
      LOGGING.info `./cnf-testsuite cnf_cleanup cnf-config=./sample-cnfs/sample_immutable_configmap_all/cnf-conformance.yml deploy_with_chart=false`
    end
  end
  ...
  it "'immutable_configmap' should pass with all immutable configmaps with env mounted", tags: ["immutable_configmap_env"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_immutable_configmap_all_plus_env/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      response_s = `./cnf-testsuite immutable_configmap verbose`
      LOGGING.info response_s
      $?.success?.should be_true
  ...
  it "'immutable_configmap' should fail with a mutable env mounted configmap", tags: ["immutable_configmap_fail"] do
    begin
      LOGGING.info `./cnf-testsuite cnf_setup cnf-config=./sample-cnfs/sample_immutable_configmap_all_plus_env_but_fail/cnf-conformance.yml deploy_with_chart=false`
      $?.success?.should be_true
      response_s = `./cnf-testsuite immutable_configmap verbose`
      LOGGING.info response_s
      $?.success?.should be_true
