...
describe "Core" do
  ...
  it "'smf_upf_heartbeat' should pass if the smf_upf core is resilient to network latency", tags: ["core"]  do
    begin
      response_s = `./cnf-testsuite cnf_setup cnf-config=sample-cnfs/sample_open5gs/cnf-testsuite.yml`
      Log.info {response_s}
  ...
  it "'suci_enabled' should fail if the 5G core does not have suci enabled", tags: ["5g"]  do
    begin
      response_s = `./cnf-testsuite cnf_setup cnf-config=sample-cnfs/sample_open5gs_no_auth/cnf-testsuite.yml`
      Log.info {response_s}
      $?.success?.should be_true
      response_s = `./cnf-testsuite suci_enabled verbose`
      Log.info {"response: #{response_s}"}
      ...
      response_s = `./cnf-testsuite cnf_cleanup cnf-config=sample-cnfs/sample_open5gs_no_auth/cnf-testsuite.yml`
      Log.info {response_s}
      $?.success?.should be_true
    end
  end
