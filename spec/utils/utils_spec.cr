...
describe "Utils" do
  ...
  it "'create_results_yml' should create a results yaml file", tags: "happy-path"  do
    create_results_yml
  ...
  it "'final_cnf_results_yml' should return the named yaml file", tags: "happy-path"  do
  ...
  it "'points_yml' should parse and return the points yaml file", tags: "happy-path"  do
  ...
  it  "'task_points(, false)' should return the amount of points for a failing test", tags: "happy-path"  do
    # default
  ...
  it "'failed_task' should find and update an existing task in the file", tags: "happy-path"  do
    create_results_yml
  ...
  it "'passed_task' should find and update an existing task in the file", tags: "happy-path"  do
    create_results_yml
  ...
  it "'task_required' should return if the passed task is required", tags: "happy-path"  do
    create_results_yml
    (task_required("privileged")).should be_true
  end

  it "'failed_required_tasks' should return a list of failed required tasks", tags: "happy-path"  do
    create_results_yml
    failed_task("privileged", "FAILURE: Privileged container found")
    (failed_required_tasks).should eq(["privileged"])
  end

  it "'upsert_task' should find and update an existing task in the file", tags: "happy-path"  do
    create_results_yml
  ...
  it "'total_points' should sum the total amount of points in the results", tags: "happy-path" do
    create_results_yml
    upsert_task("liveness", PASSED, task_points("liveness"))
  ...
  it "'tasks_by_tag' should return the tasks assigned to a tag", tags: "happy-path" do
    create_results_yml
    (tasks_by_tag("configuration_lifecycle")).should eq(["reset_cnf", "check_reaped", "versioned_helm_chart", "ip_addresses", "liveness", "readiness", "no_volume_with_configuration", "rolling_update", "nodeport_not_used", "hardcoded_ip_addresses_in_k8s_runtime_configuration"])
    (tasks_by_tag("does-not-exist")).should eq([] of YAML::Any) 
  end

  it "'all_task_test_names' should return all tasks names", tags: "happy-path" do
    create_results_yml
    (all_task_test_names()).should eq(["reasonable_image_size", "reasonable_startup_time","cni_spec", "api_snoop_alpha", "api_snoop_beta", "api_snoop_general_apis", "reset_cnf", "check_reaped", "privileged", "shells", "protected_access", "increase_capacity", "decrease_capacity", "small_autoscaling", "large_autoscaling", "network_chaos", "external_retry", "versioned_helm_chart", "ip_addresses", "liveness", "readiness", "no_volume_with_configuration", "rolling_update", "nodeport_not_used", "hardcoded_ip_addresses_in_k8s_runtime_configuration", "fluentd_traffic", "jaeger_traffic", "prometheus_traffic", "opentelemetry_compatible",  "openmetric_compatible", "helm_deploy", "install_script_helm", "helm_chart_valid", "helm_chart_published", "hardware_affinity", "static_accessing_hardware", "dynamic_accessing_hardware", "direct_hugepages", "performance", "k8s_conformance"])
  end

  it "'all_result_test_names' should return the tasks assigned to a tag", tags: "happy-path" do
    create_results_yml
    upsert_task("liveness", PASSED, task_points("liveness"))
    (all_result_test_names(LOGFILE)).should eq(["liveness"])
  end
  it "'results_by_tag' should return a list of results by tag", tags: "happy-path" do
    create_results_yml
    upsert_task("liveness", PASSED, task_points("liveness"))
  ...
  it "'toggle' should return a boolean for a toggle in the config.yml", tags: "happy-path" do
    (toggle("wip")).should eq(false) 
  end

  it "'check_feature_level' should return the feature level for an argument variable", tags: "happy-path"  do
  ...
  it "'check_<x>' should return the feature level for an argument variable", tags: "happy-path"  do
  ...
  it "'check_<x>(args)' should return the feature level for an argument variable", tags: "happy-path"  do
    args = Sam::Args.new(["name", "arg1=1", "alpha"])
