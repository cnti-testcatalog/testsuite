...
describe "Setup" do
  ...
  it "'cnf_setup/cnf_cleanup' should fail on incorrect config", tags: ["setup"] do
    begin
      result = ShellCmd.cnf_setup("cnf-path=sample-cnfs/sample-bad-config/cnf-testsuite.yml", expect_failure: true)
  ...
  it "'cnf_setup' should detect and report conflicts between deployments", tags: ["setup"] do
    begin
      result = ShellCmd.cnf_setup("cnf-path=sample-cnfs/sample-conflicting-deployments/cnf-testsuite.yml", expect_failure: true)
