...
describe "Setup" do
  ...
  it "'cnf_setup/cnf_cleanup' should fail on incorrect config", tags: ["setup"] do
    begin
      result = ShellCmd.new_cnf_setup("cnf-path=spec/fixtures/sample-bad-config.yml", expect_failure: true)
  ...
  # (svteb) TODO: change new_cnf_setup/new_cnf_cleanup to cnf_setup/cnf_cleanup in #2171 AND CHANGE TAG TO tags: ["setup"]
  it "'cnf_setup/cnf_cleanup' should install/cleanup a cnf with multiple deployments", tags: ["dummy-tag"] do
    begin
  ...
  # (svteb) TODO: change new_cnf_setup/new_cnf_cleanup to cnf_setup/cnf_cleanup in #2171 AND CHANGE TAG TO tags: ["setup"]
  it "'cnf_setup/cnf_cleanup' should install/cleanup deployment with mixed installation methods", tags: ["dummy-tag"] do
    begin
  ...
  # (svteb) TODO: change new_cnf_setup/new_cnf_cleanup to cnf_setup/cnf_cleanup in #2171 AND CHANGE TAG TO tags: ["setup"]
  it "'cnf_setup/cnf_cleanup' should handle partial deployment failures gracefully", tags: ["dummy-tag"] do
    begin
  ...
  # (svteb) TODO: change new_cnf_setup/new_cnf_cleanup to cnf_setup/cnf_cleanup in #2171 AND CHANGE TAG TO tags: ["setup"]
  it "'cnf_setup' should detect and report conflicts between deployments", tags: ["dummy-tag"] do
    begin
      result = ShellCmd.new_cnf_setup("cnf-path=spec/fixtures/sample-conflicting-deployments.yml", expect_failure: true)
