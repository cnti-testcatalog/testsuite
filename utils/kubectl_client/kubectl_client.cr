...
module KubectlClient
  ...
  module Set
    def self.image(deployment_name, container_name, image_name, version_tag=nil) : Bool
      ...
      if version_tag
        Log.debug { "kubectl set image deployment/#{deployment_name} #{container_name}=#{image_name}:#{version_tag} --record" }
        ...
        Log.debug { "kubectl set image deployment/#{deployment_name} #{container_name}=#{image_name} --record" }
        status = Process.run("kubectl set image deployment/#{deployment_name} #{container_name}=#{image_name} --record",
                             shell: true,
                             output: output = IO::Memory.new,
                             error: stderr = IO::Memory.new)
      end
      Log.info { "KubectlClient.set image output: #{output.to_s}" }
      Log.info { "KubectlClient.set image stderr: #{stderr.to_s}" }
      $?.success?
    end
  end
  ...
  module Get
    ...
    #   Log.info { "kind: #{kind}"
    #   Log.info { "resource_name: #{resource_name}"
