...
#### *To test the increasing and decreasing of capacity*: [increase_decrease_capacity](USAGE.md#increase-decrease-capacity)
> A CNF should be able to increase and decrease its capacity without running into errors.

#### *Test if the Helm chart is published*: [helm_chart_published](USAGE.md#helm-chart-published)
...
#### *Test if the Helm chart is valid*: [helm_chart_valid](USAGE.md#helm-chart-is-valid)
> A chart should pass the [lint specification](https://helm.sh/docs/helm/helm_lint/#helm)

#### *Test if the Helm deploys*: [helm_deploy](USAGE.md#helm-deploy)
> A helm chart should be [deployable to a cluster](https://helm.sh/docs/helm/helm_install/#helm)

#### *To check if a CNF version can be rolled back*: [rollback](USAGE.md#rollback)
...
#### *To test if the CNF can perform a rolling update*: [rolling_update](USAGE.md#rolling-update)
> See rolling downgrade

#### *To check if a CNF version can be downgraded through a rolling_version_change*: [rolling_version_change](USAGE.md#rolling-version-change)
> See rolling downgrade

#### *To check if a CNF version can be downgraded through a rolling_downgrade*: [rolling_downgrade](USAGE.md#rolling-downgrade)
...
#### *To check if the CNF is compatible with different CNIs*: [cni_compatibility](USAGE.md#cnf-compatible)
...
#### *[POC] To check if a CNF uses Kubernetes alpha APIs 'alpha_k8s_apis'*: [alpha_k8s_apis](USAGE.md#kubernetes-alpha-apis)
...
#### *To check if the CNF has a reasonable image size*: [reasonable_image_size](USAGE.md#reasonable-image-size)

> A CNF with a large image size of 5 gig or more tends to indicate a monolithic application

#### *To check if the CNF have a reasonable startup time*: [reasonable_startup_time](USAGE.md#reasonable-startup-time)
...
#### *To check if the CNF has multiple process types within one container*: [single_process_type](USAGE.md#single-process-type-in-one-container)
...
#### *To check if the CNF exposes any of its containers as a service 'service_discovery'*: [service_discovery](USAGE.md#service-discovery)
...
#### *To check if the CNF uses a shared database*: [shared_database](USAGE.md#shared-database)
...
#### *Test if the CNF crashes when node drain occurs*: [node_drain](USAGE.md#node-drain)
...
#### *To test if the CNF uses a volume host path*: [volume_hostpath_not_found](USAGE.md#volume-hostpath-not-found)

> When a cnf uses a volume host path or local storage it makes the application tightly coupled 
to the node that it is on.  

#### *To test if the CNF uses local storage*: [no_local_volume_configuration](USAGE.md#no-local-volume-configuration)
> A CNF should refrain from using the [local storage class](https://kubernetes.io/docs/concepts/storage/storage-classes/#local)

#### *To test if the CNF uses elastic volumes*: [elastic_volumes](USAGE.md#elastic-volumes)

> A cnf that uses elastic volumes can be rescheduled to other nodes by the orchestrator easily

#### *To test if the CNF uses a database with either statefulsets, elastic volumes, or both*: [database_persistence](USAGE.md#database-persistence)
...
#### *Test if the CNF crashes when network latency occurs*: [pod_network_latency](USAGE.md#cnf-network-latency)
...
#### *Test if the CNF crashes when disk fill occurs*: [disk_fill](USAGE.md#cnf-disk-fill)
...
#### *Test if the CNF crashes when pod delete occurs*: [pod_delete](USAGE.md#pod-delete)
...
#### *Test if the CNF crashes when pod memory hog occurs*: [pod_memory_hog](USAGE.md#memory-hog)
...
#### *Test if the CNF crashes when pod io stress occurs*: [pod_io_stress](USAGE.md#io-stress)
...
#### *Test if the CNF crashes when pod network corruption occurs*: [pod_network_corruption](USAGE.md#network-corruption)
...
#### *Test if the CNF crashes when pod network duplication occurs*: [pod_network_duplication](USAGE.md#network-duplication)
...
#### *To test if there is a liveness entry in the Helm chart*: [liveness](USAGE.md#helm-chart-liveness-entry)
...
#### *To test if there is a readiness entry in the Helm chart*: [readiness](USAGE.md#helm-chart-readiness-entry)
...
#### *To check if logs are being sent to stdout/stderr (standard out, standard error) instead of a log file*: [log_output](USAGE.md#use-stdoutstderr-for-logs)
...
#### *To check if prometheus is installed and configured for the cnf*: [prometheus_traffic](USAGE.md#prometheus-installed)
...
#### *To check if logs and data are being routed through fluentd*: [routed_logs](USAGE.md#fluentd-logs)
...
#### *To check if OpenMetrics is being used and or compatible.*: [open_metrics](USAGE.md#openmetrics-compatible)
...
#### *To check if tracing is being used with Jaeger.*: [tracing](USAGE.md#jaeger-tracing)
...
#### *To check if the cnf performs a CRI socket mount*: [container_sock_mounts](USAGE.md#container-socket-mounts)
...
#### *To check if there are any privileged containers*: [privileged](USAGE.md#privileged-mode)
...
#### *To check if External IPs are used for services*: [external_ips](USAGE.md#external-ips)
...
#### *To check if any containers are running as a root user (checks the user outside the container that is running dockerd)*: [non_root_user](USAGE.md#root-user)
...
#### *To check if any containers allow for privilege escalation*: [privilege_escalation](USAGE.md#privilege-escalation)
...
#### *To check if an attacker can use a symlink for arbitrary host file system access (CVE-2021-25741)*: [symlink_file_system](USAGE.md#symlink-file-system)
...
#### *To check if there is a host network attached to a pod*: [host_network](USAGE.md#host-network)
...
#### *To check if there is an ingress and egress policy defined.*: [ingress_egress_blocked](USAGE.md#ingress-and-egress-blocked)
...
#### *To check if there are any privileged containers (kubscape version)*: [privileged_container](USAGE.md#privilege-container)
...
#### *To check for insecure capabilities*: [insecure_capabilities](USAGE.md#insecure-capabilities)
...
#### *To check for dangerous capabilities*: [dangerous_capabilities](USAGE.md#dangerous-capabilities)
...
#### *To check if namespaces have network policies defined*: [network_policies](USAGE.md#network-policies)
...
#### *To check if containers are running with non-root user with non-root membership*: [non_root_containers](USAGE.md#non-root-containers)
...
#### *To check if containers are running with hostPID or hostIPC privileges*: [host_pid_ipc_privileges](USAGE.md#host-pidipc-privileges)
...
#### *To check if security services are being used to harden containers*: [linux_hardening](USAGE.md#linux-hardening)
...
#### *To check if containers have resource limits defined*: [resource_policies](USAGE.md#resource-policies)
...
#### *To check if containers have immutable file systems*: [immutable_file_systems](USAGE.md#immutable-file-systems)
...
#### *To check if containers have hostPath mounts (check: is this a duplicate of state test - ./cnf-testsuite volume_hostpath_not_found)*: [hostpath_mounts](USAGE.md#hostpath-mounts)
...
#### *To check if selinux has been configured properly*: [selinux_options](USAGE.md#selinux-options)
...
#### *To test if mutable tags being used for image versioning(Using Kyverno): latest_tag*: [latest_tag](USAGE.md#latest-tag)
...
#### *To test if the recommended labels are being used to describe resources*: [required_labels](USAGE.md#require-labels)
...
#### *To test if there are any (non-declarative) hardcoded IP addresses or subnet masks in the Helm Chart*: [ip_addresses](USAGE.md#ip-addresses)
...
#### *To test if there are node ports used in the service configuration*: [nodeport_not_used](USAGE.md#nodeport-not-used)

> Using node ports ties the CNF to a specific node and therefore makes the CNF less
portable and scalable

#### *To test if there are host ports used in the service configuration*: [hostport_not_used](USAGE.md#hostport-not-used)

> Using host ports ties the CNF to a specific node and therefore makes the CNF less
portable and scalable

#### *To test if there are any (non-declarative) hardcoded IP addresses or subnet masks in the K8s runtime configuration*: [hardcoded_ip_addresses_in_k8s_runtime_configuration](USAGE.md#hardcoded-ip-addresses-in-k8s-runtime-configuration)

> Using a hard coded IP in a CNF's configuration designates *how* (imperative) a CNF should 
achieve a goal, not *what* (declarative) goal the CNF should achieve

#### *To check if a CNF uses K8s secrets*: [secrets_used](USAGE.md#secrets-used)
...
#### *To check if a CNF version uses immutable configmaps*: [immutable_configmap](USAGE.md#immutable-configmaps)
